...
//  calculating the interaction force from IVC onto SVC, therefore atom i, requiring  IVC_id[i]>0 && IVC_id[j]<0 
...
//    printf("atomcalled, ii %d ilist[ii] %d,group 1->cont %d,IVC_id %d \n",ii,i,(mask[i]&groupbit),IVC_id[i]);
    ...
    if (IVC_id[i] <= 0) continue;        // i atom should be SVC, i.e. IVC_id[i] > 0
    xtmp = x[i][0];
    ytmp = x[i][1];
    ztmp = x[i][2];
    itype = type[i];
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if(IVC_id[j] >= 0) continue;	// j atom should be IVC, i.e. IVC_id[i] < 0
