...
enum{PF_CALLBACK,PF_ARRAY,PF_ESCM,PF_IVCDAMP,PF_ESCM_FP};
enum{SCALAR,VECTOR};
  ...
  Fix(lmp, narg, arg)
{
  ...
  if (strcmp(arg[3],"pf/callback") == 0) {
    ...
    if(narg == 8){
       ...
       // arg [1]: ncall [2]: napply [3]:nnode_inter [4]:use Fp
       ncall = force->inumeric(FLERR,arg[4]);
       napply = force->inumeric(FLERR,arg[5]);
       nstore = force->inumeric(FLERR,arg[6]);
       nnode_inter = force->inumeric(FLERR,arg[7]);
       if(strcmp(arg[8],"Fp") == 0){
          mode = PF_ESCM_FP;
       }else{
          error->all(FLERR,"ESCM_FP mode must use 'Fp' as argument");
       }
    }else{
       error->all(FLERR,"Illegal fix external command, wrong arg number");
    }
     ...
     if(ncall > 0){
       ...
       if(icompute==-1){
          ...
          if(nrows!=nnode_inter*3){
            if(me==0) printf("wrong nrows is %d not equal to 3*nnodeinter: 3*%d \n",nrows,nnode_inter);
...
//        if(me==0)  printf("initial value of fs0[%d] is %f\n",i,fs0[i]);
     ...
     int nplanes = 4;
     int nFp = 9;
     nvalues2 = nnode_inter*nplanes*nFp;
     int length2;
     char* compute_id2 = "IVCFp";
     IVC_Fp0 = NULL;
     memory->grow(IVC_Fp0,nvalues2,"external:IVC_Fp0");
     for(int i =0;i<nvalues2;i++){
         IVC_Fp0[i] = 0.0;
     }
     flag_Fp_exist = 1;
     if(ncall > 0){
        icompute2 = modify->find_compute(compute_id2);
        if(icompute2 == -1){
          error->warning(FLERR,"fix external ESCM mode without compute IVCFp");
          flag_Fp = 0;     
        }else{
          flag_Fp = 1;
          length2 = modify->compute[icompute2]->size_vector;
          //if(me==0)   printf("IVCFp has vector size %d\n",length2);
          nrows2 = length2;
          column2 = new double[nrows2];
        }
     }

     nvalid_call = nextvalid_call();
     modify->addstep_compute_all(nvalid_call);         // not quite sure how this is working for individual computes, 
                                                       // take a look at tlist and tflag in compute.h/cpp
...
//            if(me==0) printf("in init compute added at step %d\n",nvalid_call);
   ...
   //     if(me==0)  printf("in init icompute is %d\n",icompute);
       ...
       if(flag_Fp == 1){
          char* compute_id2 ="IVCFp" ;   // use preesure to test
          icompute2 = modify->find_compute(compute_id2);
   //     if(me==0)  printf("in init icompute is %d\n",icompute);
        // icompute is same value for same compute like c_p[1] c_p[2]
          if(icompute2<0){
             error->all(FLERR,"Compute ID for fix external using IVCFp not found");
          }
//            if(me==0) printf("in init compute added at step %d\n",nvalid_call);
       }
       if((flag_SVC==1)&&(flag_Fp==1)){
...
//      if(me == 0)  printf("at step %d end of step is called\n",ntimestep);
     ...
     if(ntimestep == nstore){
       ...
       for(i=0;i<nvalues;i++){
        ...
        if(flag_fs0_exist == 1)     distribute_force_SVC();
        int nt = (int) ntimestep;
        if(me==0) printf("end of step is done at step %d\n",nt);
     }
   }else if(mode = PF_IVCDAMP){
     if(ntimestep == nstore){
...
//   icompute = modify->find_compute(compute_id);
   if(flag_SVC == 1){
      m1 = icompute;  
      compute_SVCf = modify->compute[m1];
      cvector1 = compute_SVCf->vector;
      if(!(compute_SVCf->invoked_flag & INVOKED_VECTOR)){
         compute_SVCf->compute_vector();
         compute_SVCf->invoked_flag |= INVOKED_VECTOR;
      }
   }
...
void FixExternal::invoke_vector2(bigint ntimestep)
{
   int i,m2;
   Compute *compute_IVCFp;
   double *cvector2;
//   printf("compute_vector is called at step %d in proc:%d\n",ntimestep,comm->me);
//   if(me==0) printf("invoke_vector is called at timestep %d, nvalues %d\n",ntimestep,nvalues);
   if(irepeat == 0){
      if(flag_Fp == 1)   for(i=0;i<nvalues2;i++) column2[i] = 0.0;
   }
   modify->clearstep_compute();
...
//   icompute = modify->find_compute(compute_id);
   if(flag_Fp == 1){
      m2 = icompute2;
      compute_IVCFp = modify->compute[m2];
      cvector2 = compute_IVCFp->vector;
      if(!(compute_IVCFp->invoked_flag & INVOKED_VECTOR)){
         compute_IVCFp->compute_vector();
         compute_IVCFp->invoked_flag |= INVOKED_VECTOR;
      }
   }
   if(flag_Fp == 1){
      for (i=0;i<nrows2;i++){
         column2[i] = cvector2[i];
      }
   }
   /*
   irepeat ++;
...
//    printf("apply is called in post_force at timestep %d\n",ntimestep);
  ...
  if(((mode == PF_ESCM)||(mode==PF_ESCM_FP))&&(ntimestep % napply == 0)) {
//    printf("apply is called in post_force at timestep %d\n",ntimestep);
    double **f = atom->f;
    int *mask = atom->mask;
    int nlocal = atom->nlocal;
