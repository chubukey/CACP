...
//  double **neighlist0 = fix2->array_atom;
  ...
  double dxprd = domain->xprd-xprd0;
  double dyprd = domain->yprd-yprd0;
  double dzprd = domain->zprd-zprd0;
  double cross[3];
  ...
  int neighcount,elcount,flag_inv[1];
  flag_inv[0] = 1;
  ...
  for(i=0;i<nlocal;i++){
     ...
     for(j=0;j<nneigh;j++){
        ...
        if(n_id<0){
        // this means this atom originally has less than 12 neighbors
//          printf("wrong id for atom %d neigh %d, id_global %d, id_local %d check neighlist build\n",i,j,n_id_global,n_id);
        }else{
          xn[0] = xa[n_id][0] ;
          xn[1] = xa[n_id][1] ;
          xn[2] = xa[n_id][2] ;
          ...
          wtf = 3.52*(1.0-1.0/48.0)/1000;
          wtf = 0.0;
          distT[0] = distT[0] - cross[0]*(dxprd-wtf);
          distT[1] = distT[1] - cross[1]*(dyprd-wtf);
          distT[2] = distT[2] - cross[2]*(dzprd-wtf);
